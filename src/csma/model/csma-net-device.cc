  ...
  else
    {
      ...
      if (m_channel->TransmitStart (m_currentPkt, m_deviceId) == false)
      ...
      else
        {
          ...
          NS_LOG_INFO ("CsmaNetDevice: " << m_address << ": Started transmission of frame with size: " << m_currentPkt->GetSize() << ", serialized size: " << m_currentPkt->GetSerializedSize());
          // NS_LOG_INFO ("  Frame: " << m_currentPkt->ToString());
...
CsmaNetDevice::Receive (Ptr<Packet> packet, Ptr<CsmaNetDevice> senderDevice)
{
  ...
  NS_LOG_INFO (Simulator::Now());
  NS_LOG_INFO ("CsmaNetDevice: " << m_address << ": Received frame with size: " << originalPacket->GetSize() << ", serialized size: " <<
               originalPacket->GetSerializedSize() << ", from device address: " << senderDevice->GetAddress());
  // NS_LOG_INFO ("  Frame: " << originalPacket->ToString());
...
CsmaNetDevice::SendFrom (Ptr<Packet> packet, const Address& src, const Address& dest, uint16_t protocolNumber)
{
  ...
  if (m_queue->Enqueue (packet) == false)
    {
      NS_LOG_INFO ("Could not enqueue packet. Dropped.");
      m_macTxDropTrace (packet);
      return false;
    }
      ...
      if (m_queue->IsEmpty () == false)
        {
          ...
          m_promiscSnifferTrace (m_currentPkt);
          m_snifferTrace (m_currentPkt);
          NS_LOG_INFO ("TransmitStart call.");
          TransmitStart ();
        }
    }
  return true;

...
{
  NS_LOG_FUNCTION (this << maxSize << flags);
  NS_LOG_INFO ("UdpSocketImpl: Node " << m_node->GetId () << ", about to get from delivery queue packet from: " << &fromAddress << ", max.size: " << maxSize);
  NS_LOG_INFO ("Flags: " << flags << ", from: " << fromAddress);
                          ...
                          Ptr<Ipv4Interface> incomingInterface)
{
  ...
  NS_LOG_INFO ("UdpSocketImpl: Node " << m_node->GetId() << ", forwarding up packet " << packet-> GetUid () << " with size: " << packet->GetSize () << ", serialized size: " << packet->GetSerializedSize ());
  NS_LOG_INFO ("Packet: " << packet->ToString ());
  NS_LOG_INFO ("Header: " << header << " port: " << port << " incomingIface: " << incomingInterface);
  if (m_shutdownRecv)
    {
      return;
    }

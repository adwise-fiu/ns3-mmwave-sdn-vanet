...
main (int argc, char *argv[])
{
  ...
  NS_LOG_INFO ("Ipv4 address of port 6: " << switchNode->GetObject<Ipv4>()->GetAddress(4,0).GetLocal());
  ...
  // sinkApps.Add (ulsinkHelper.Install (wifi80211pNodes.Get (0)));
  // sinkApps.Add (ulsinkHelper.Install (hosts.Get(0)));
  ...
  // OnOffHelper srcHost ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address("20.1.1.1"), ulPort));
  ...
  OnOffHelper srcServer ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address("7.0.0.4"), dlPort));
  ...
  // srcApps.Add (srcServer.Install(hosts.Get(0)));
